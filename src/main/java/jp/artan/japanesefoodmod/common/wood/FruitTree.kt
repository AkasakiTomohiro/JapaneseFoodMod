 ...
 *      テクスチャ
 */
abstract class FruitTree(val fruit: JapaneseFood, val log: FruitLog, val leaves: FruitLeaves, val sapling: FruitSapling, val fruitBlock: FruitBlock) {
    init {
        CustomBiome.fruits.add(this)
    }

    fun registerOreDictionaryEntries() {
        OreDictionary.registerOre("logWood", ItemStack(this.log, 1, OreDictionary.WILDCARD_VALUE));
