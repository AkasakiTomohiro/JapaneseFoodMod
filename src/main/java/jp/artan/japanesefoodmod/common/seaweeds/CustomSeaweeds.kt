 ...
 *      テクスチャ
 */
abstract class CustomSeaweeds(val food: JapaneseFood, val block: CustomSeaweedBlock, val worldGen: WorldGenSeaweed) {

    init {
        this.block.item = this.food
        this.worldGen.setBlockStateWood(this.block)
    }

    fun getWorldGenerator(flag: Boolean): WorldGenSeaweed {
